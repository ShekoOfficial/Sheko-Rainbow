exports.run = async(bot, message, args,) => {
  ...
  const queue = bot.queue;
		...
		if (!message.member.voiceChannel) return message.reply('You are not in the same voice channel as me.');
		if (!serverQueue || !serverQueue.songs.length) return message.reply('There is nothing to shuffle');

		serverQueue.songs = fixedAllDifferentShuffle(serverQueue.songs, [true]);

		message.reply('The queue jas been shuffled');
}
